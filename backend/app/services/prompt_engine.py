...
def _build_char_range_instruction(content_type: str, target: int, tolerance: float = 0.04) -> str:
    """目標±tolerance% の帯域指示文を生成"""
    min_c = math.floor(target * (1 - tolerance))
    max_c = math.ceil(target * (1 + tolerance))
    mode = CONTENT_TYPES.get(content_type, {}).get("char_count_mode", "hpb")
    label = "HPB基準（全角1文字、半角0.5文字）" if mode == "hpb" else "素の文字数"
    return (
        f"## 文字数\n"
        f"- 目標: {target}文字（{label}）\n"
        f"- 許容範囲: {min_c}〜{max_c}文字（目標±{int(tolerance * 100)}%）\n"
        f"- この範囲内に必ず収めてください。"
    )
...
def build_system_prompt(
    ...
    Args:
        ...
        target_char_count: 目標文字数（未指定時はmax_charsを使用）
    ...
    if content_type == "review_reply":
        target = target_char_count or 500
        char_instruction = _build_char_range_instruction(content_type, target)
...
- 次回来店への期待を込める
- 以下の禁止表現は使用しない: {', '.join(PROHIBITED_EXPRESSIONS[:5])}など
- 完成した返信文のみを出力

{char_instruction}{dialect_instruction}"""
...
{char_instruction}{dialect_instruction}"""
    ...
    if content_type == "consultation":
        target = target_char_count or 2000
        char_instruction = _build_char_range_instruction(content_type, target)
...
- 完成したテキストのみを出力してください
- 説明や補足は不要です
- ハッシュタグ（#xxx）は本文に含めないでください（サロン固有のハッシュタグは別途自動付与されます）
{dialect_instruction}"""
...
def build_full_prompt(
    ...
    parts.append(build_system_prompt(content_type, stylist, target_char_count=target_char_count))

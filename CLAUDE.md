...
## ルール
明示がなければ、テストと言われた時は本番アプリの環境でテストして
Chrome Devtoolsを基本使うこと。Playwrightは使わない。
...
└── backend/                           # FastAPI アプリ
    ...
    │   │   ├── billing.py
    │   │   ├── usage.py
    │   │   ├── organizations.py       # Teamプラン
    │   │   ├── csv_import.py          # CSV一括登録
    │   │   └── public.py              # 公開API（認証不要）
    │   ├── services/
    │   │   ├── claude_service.py      # Claude API（テキスト生成）
    │   │   ├── gemini_service.py      # Gemini API（画像用、Phase 2）
    │   │   ├── nanobanana_service.py
    │   │   ├── prompt_engine.py
    │   │   ├── content_service.py
    │   │   ├── stripe_service.py
    │   │   ├── usage_service.py
    │   │   ├── organization_service.py  # Teamプラン
    │   │   ├── csv_import_service.py    # CSV一括登録
...
**実装済み:**
- ユーザー認証（Supabase Auth: メール+パスワード / 開発時mock_auth）
- サロン・スタイリスト登録（フォーム形式）
- テキスト生成 7種: サロンキャッチ、サロン紹介文、スタイリストプロフィール、ブログ記事、口コミ返信、カウンセリング文、Google口コミ返信
- チャット修正機能（正規化済み: chat_sessions + chat_messages）
- コピーボタン付き出力画面 + HPB文字数カウンター
- 利用量制限（Free: テキスト月30回/ブログ月5回、Pro/Team: 無制限）
- ダッシュボード + コンテンツ履歴
- Teamプラン（組織管理、CSV一括登録、メンバー管理）
- 管理者CLIツール（backend/cli/）
- 公開統計API（/api/v1/public/stats）
...
### Phase 2（6〜7月）
オンボーディングチャット、AI画像、一括生成、Stripe決済実装、エクスポート、追加コンテンツタイプ
...
- `salons` — サロン情報（user_id, organization_id, name, area, concept, target_customer, strength）
- `stylists` — スタイリスト + メタデータ（name, role, specialties, style_features, personality, writing_style）
- `generated_contents` — 生成コンテンツ（content, char_count, prompt_used, input_tokens, output_tokens）
- `chat_sessions` + `chat_messages` — チャット修正履歴（正規化済み）
- `subscriptions` — Stripe連携サブスク状態（plan: free/pro/team）
- `usage_tracking` — 月間利用量（year_month単位で集計）
- `organizations` + `organization_members` + `csv_import_jobs` — Teamプラン用
- ※ `users` テーブルはなし（Supabase Auth管理）
- ※ `prompt_templates` テーブルはなし（prompt_engine.pyでコード管理）
...
```
[システムプロンプト（content_type別、prompt_engine.pyで管理）]
  + [サロン情報（name, area, concept, target_customer, strength）]
  + [スタイリストメタデータ（specialties, style_features, personality, writing_style）]
  + [HPB制約（文字数上限、禁止表現）]
  + [コンテンツ固有データ（blog_theme, review_text, consultation_text, star_rating）]
  + [ユーザーの追加指示]
  → Claude 4.5 Haiku API（SSEストリーミング）
```

- テンプレートは`prompt_engine.py`にコード管理（将来DB移行予定）
...
3. **プロンプトは現在コード管理（prompt_engine.py）。** 将来的にDB管理に移行し、A/Bテスト・チューニングに対応予定。
